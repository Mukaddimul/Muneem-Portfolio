...
const AdminPanel: React.FC<AdminPanelProps> = ({ data, onSave, onLogout, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#111827] flex flex-col md:flex-row overflow-hidden animate-in fade-in duration-300 font-sans selection:bg-blue-600/30 selection:text-white">
      ...
      >
        {/* General Profile section remains same as previous */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div className="space-y-10">
              <div>
                ...
                <input value={editData.profile.fullName} onChange={(e) => setEditData({...editData, profile: {...editData.profile, fullName: e.target.value}})} className="w-full bg-[#1e293b] border-2 border-slate-800/40 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-blue-500 transition-all shadow-inner" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 block">Short Name</label>
                <input value={editData.profile.name} onChange={(e) => setEditData({...editData, profile: {...editData.profile, name: e.target.value}})} className="w-full bg-[#1e293b] border-2 border-slate-800/40 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-blue-500 transition-all shadow-inner" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 block">Location</label>
                <input value={editData.profile.location} onChange={(e) => setEditData({...editData, profile: {...editData.profile, location: e.target.value}})} className="w-full bg-[#1e293b] border-2 border-slate-800/40 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-blue-500 transition-all shadow-inner" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 block">Bio</label>
                <textarea value={editData.profile.bio} onChange={(e) => setEditData({...editData, profile: {...editData.profile, bio: e.target.value}})} rows={5} className="w-full bg-[#1e293b] border-2 border-slate-800/40 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-blue-500 transition-all resize-none leading-relaxed shadow-inner" />
              </div>
            </div>
            <div className="space-y-12">
              <div className="p-8 bg-[#1e293b]/30 rounded-[2.5rem] border border-slate-800/50">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-6 block">Profile Picture</label>
              ...
              <div className="p-8 bg-[#1e293b]/30 rounded-[2.5rem] border border-slate-800/50">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-6 block">Cover Photo</label>
                <div className="flex items-center gap-8">
        ...
        {/* Other sections (skills, projects, experience) remain largely same as before, skipping for brevity but they are present in full app logic */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {editData.skills.map((skill, idx) => (
              <div key={skill.id} className="bg-[#1e293b] p-10 rounded-[2.5rem] border border-slate-800/50 shadow-2xl group relative hover:border-blue-500/20 transition-all duration-500">
                <button onClick={() => setEditData({...editData, skills: editData.skills.filter(s => s.id !== skill.id)})} className="absolute top-8 right-8 text-slate-700 hover:text-red-500 transition-colors"><i className="fa-solid fa-trash-can text-lg"></i></button>
                <div className="flex items-center gap-4 mb-8">
                   <input value={skill.title} onChange={(e) => { const newSkills = [...editData.skills]; newSkills[idx].title = e.target.value; setEditData({ ...editData, skills: newSkills }); }} className="bg-transparent border-none text-2xl font-black text-white p-0 focus:outline-none flex-1" placeholder="Category Name" />
                </div>
                <div className="space-y-4 mb-8">
                  {skill.items.map((item, itemIdx) => (
                    <div key={itemIdx} className="flex items-center gap-4 group/item">
                      <input value={item} onChange={(e) => { const newSkills = [...editData.skills]; newSkills[idx].items[itemIdx] = e.target.value; setEditData({ ...editData, skills: newSkills }); }} className="flex-1 bg-[#111827]/50 border-2 border-slate-800/50 rounded-xl px-5 py-3 text-sm text-slate-300 font-bold outline-none focus:border-blue-500/50 transition-all" />
                      <button onClick={() => { const newSkills = [...editData.skills]; newSkills[idx].items = newSkills[idx].items.filter((_, i) => i !== itemIdx); setEditData({ ...editData, skills: newSkills }); }} className="text-slate-600 hover:text-slate-400 transition-colors"><i className="fa-solid fa-xmark"></i></button>
                    </div>
                  ))}
                </div>
                <button onClick={() => { const newSkills = [...editData.skills]; newSkills[idx].items.push('New Skill'); setEditData({ ...editData, skills: newSkills }); }} className="text-blue-500 hover:text-blue-400 font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-colors">+ Add Skill</button>
              </div>
            ))}
          </div>
        </section>

        {/* RESTORED AWARDS SECTION MATCHING SCREENSHOT */}
        <section id="admin-section-awards" className="scroll-mt-20">
          <div className="flex items-center justify-between mb-12">
            ...
            <button 
              onClick={() => {
                const newAward: Award = { id: Date.now().toString(), title: 'International Leadership Award', detail: '2023 Recipient', icon: 'fa-earth-americas' };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {editData.awards.map((award, aIdx) => (
              <div key={award.id} className="bg-[#1e293b]/40 p-10 rounded-[2.5rem] border border-slate-800/50 shadow-2xl relative flex gap-8 items-center group transition-all hover:bg-[#1e293b]/60">
                {/* Delete Button (Visible on hover) */}
                <button 
                  onClick={() => setEditData({...editData, awards: editData.awards.filter(a => a.id !== award.id)})}
                  className="absolute top-6 right-6 text-red-500/30 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"
                >
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className="w-20 h-20 rounded-[1.5rem] bg-[#111827] border border-slate-800 flex items-center justify-center text-3xl text-slate-400 shrink-0 shadow-inner group-hover:text-brand-400 transition-colors">
                    <i className={`fa-solid ${award.icon}`}></i>
                  </div>
                  <div className="w-full">
                    <label className="text-[8px] font-black uppercase text-slate-600 mb-1 block text-center">Icon Class</label>
                    <input 
                      value={award.icon} 
                      onChange={(e) => {
                      ...
                      className="w-full bg-[#111827] border border-slate-800/50 rounded-lg px-2 py-1 text-[9px] text-brand-500/80 font-mono focus:outline-none text-center" 
                ...
                <div className="flex-1 space-y-4">
                  <div className="relative group/field">
                    <label className="text-[9px] font-black uppercase text-slate-600 mb-1.5 block ml-1 tracking-widest">Award Title</label>
                    <input 
                      value={award.title} 
                      onChange={(e) => {
                        const newAwards = [...editData.awards];
                      ...
                      className="w-full bg-[#111827]/80 border border-slate-800/80 rounded-2xl px-6 py-4 text-white font-bold outline-none focus:border-brand-500/50 transition-all shadow-inner text-sm" 
                      placeholder="e.g. International Leadership Award"
                    />
                  </div>
                  <div className="relative group/field">
                    <label className="text-[9px] font-black uppercase text-slate-600 mb-1.5 block ml-1 tracking-widest">Achievement Detail</label>
                    <input 
                      value={award.detail} 
                      onChange={(e) => {
                        const newAwards = [...editData.awards];
                      ...
                      className="w-full bg-[#111827]/80 border border-slate-800/80 rounded-2xl px-6 py-4 text-brand-400 font-bold outline-none focus:border-brand-500/50 transition-all shadow-inner text-xs" 
                      placeholder="e.g. 2023 Recipient / Nominated"
                    />
                  </div>
                </div>
              </div>
        ...
        {/* Project and other sections continue here... skipping for brevity as they are unchanged from previousTurn but remain in component */}
        <section id="admin-section-news" className="scroll-mt-20">
          <div className="flex items-center justify-between mb-12">
            <div><h3 className="text-3xl font-black text-white uppercase tracking-tight mb-4">News Archive</h3><div className="h-[2px] w-48 bg-slate-800"></div></div>
            <button onClick={() => { const newPost: NewsPost = { id: Date.now().toString(), title: 'Breaking News Headline', content: 'Story details...', author: 'News Desk', date: new Date().toISOString(), likes: 0, comments: [], images: [] }; setEditData({ ...editData, news: [newPost, ...editData.news] }); }} className="bg-[#2563eb] hover:bg-blue-700 text-white px-8 py-3.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-blue-600/20 active:scale-95">+ News Post</button>
          </div>
          <div className="space-y-12">
            {editData.news.map((post, nIdx) => (
              <div key={post.id} className="bg-[#1e293b] p-10 md:p-14 rounded-[2.5rem] border border-slate-800/50 shadow-2xl relative">
                <button onClick={() => setEditData({...editData, news: editData.news.filter(p => p.id !== post.id)})} className="absolute top-10 right-10 text-red-500 hover:text-red-400 transition-colors"><i className="fa-solid fa-trash-can text-xl"></i></button>
                <div className="space-y-6 mb-8">
                  <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block ml-2">Headline</label><input value={post.title} onChange={(e) => { const newNews = [...editData.news]; newNews[nIdx].title = e.target.value; setEditData({ ...editData, news: newNews }); }} className="w-full bg-[#111827] border-2 border-slate-800/50 rounded-2xl px-6 py-4 text-white font-black italic uppercase tracking-tighter outline-none focus:border-blue-500 transition-all text-xl shadow-inner" placeholder="Headline..." /></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block ml-2">Author</label><input value={post.author} onChange={(e) => { const newNews = [...editData.news]; newNews[nIdx].author = e.target.value; setEditData({ ...editData, news: newNews }); }} className="w-full bg-[#111827] border-2 border-slate-800/50 rounded-xl px-5 py-3 text-sm text-slate-200 font-bold outline-none focus:border-blue-500/50 shadow-inner" placeholder="e.g. News Desk" /></div>
                    <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block ml-2">Publication Date</label><input type="date" value={post.date.split('T')[0]} onChange={(e) => { const newNews = [...editData.news]; newNews[nIdx].date = new Date(e.target.value).toISOString(); setEditData({ ...editData, news: newNews }); }} className="w-full bg-[#111827] border-2 border-slate-800/50 rounded-xl px-5 py-3 text-sm text-blue-400 font-bold outline-none focus:border-blue-500/50 shadow-inner [color-scheme:dark]" /></div>
                  </div>
                  <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block ml-2">Article Content</label><textarea value={post.content} onChange={(e) => { const newNews = [...editData.news]; newNews[nIdx].content = e.target.value; setEditData({ ...editData, news: newNews }); }} rows={6} className="w-full bg-[#111827]/50 border-2 border-slate-800/50 rounded-[1.5rem] px-6 py-4 text-slate-300 font-bold outline-none focus:border-blue-500 transition-all text-sm leading-relaxed shadow-inner" placeholder="Article text..." /></div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block">Photo Gallery</label><span className="text-[9px] font-bold text-slate-600 uppercase">{(post.images || []).length} / 6</span></div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                    {(post.images || []).map((img, iIdx) => (
                      <div key={iIdx} className="relative aspect-square rounded-xl bg-[#111827] border border-slate-800 group overflow-hidden"><img src={img} className="w-full h-full object-cover" alt={`Post image ${iIdx + 1}`} /><div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-opacity"><button onClick={() => deleteNewsImage(post.id, iIdx)} className="text-white hover:text-red-400 transition-colors w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center backdrop-blur-md"><i className="fa-solid fa-trash text-sm"></i></button></div></div>
                    ))}
                    {(post.images || []).length < 6 && (
                      <label className="aspect-square rounded-xl bg-slate-800/50 border-2 border-dashed border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-800 hover:border-slate-500 transition-all group">
                        <input type="file" className="hidden" accept="image/*" onChange={(e) => handleNewsPhotoUpload(post.id, e)} /><i className="fa-solid fa-plus text-slate-500 mb-2 group-hover:scale-125 transition-transform"></i><span className="text-[9px] font-black text-slate-600 uppercase">Add Photo</span>
                      </label>
                    )}
                  </div>
                </div>
      ...
      {/* Modern Cropper Modal remains same */}
      {cropModal.isOpen && (
        <div className="fixed inset-0 z-[200] bg-[#0f172a] flex flex-col p-4 md:p-10">
          <div className="flex-1 relative rounded-[3rem] overflow-hidden border-4 border-slate-800 shadow-2xl">
            <Cropper image={cropModal.image} crop={crop} zoom={zoom} aspect={cropModal.field === 'profilePic' || (typeof cropModal.field === 'object' && (cropModal.field.type === 'experience' || cropModal.field.type === 'project')) ? 1 : 16 / 9} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={onCropComplete} />
          </div>
          <div className="p-8 bg-[#1e293b] border-2 border-slate-800 mt-6 rounded-[2.5rem] flex flex-col md:flex-row gap-8 justify-center items-center shadow-2xl">
             <div className="flex-1 max-w-sm"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4 block text-center">Adjust Zoom Level</label><input type="range" value={zoom} min={1} max={3} step={0.1} onChange={(e) => setZoom(Number(e.target.value))} className="w-full accent-blue-600" /></div>
             <div className="flex gap-4"><button onClick={() => setCropModal({ ...cropModal, isOpen: false } as any)} className="px-10 py-4 bg-slate-800 text-white rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-slate-700 transition-colors border border-slate-700 active:scale-95">Cancel</button><button onClick={handleCropSave} disabled={isProcessingImage} className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-blue-500 shadow-xl shadow-blue-600/30 disabled:opacity-50 transition-all active:scale-95 flex items-center gap-3">{isProcessingImage ? <i className="fa-solid fa-spinner animate-spin"></i> : <i className="fa-solid fa-check"></i>}Finalize & Save</button></div>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        input[type="range"] { height: 6px; -webkit-appearance: none; background: #334155; border-radius: 5px; }
      `}</style>
    </div>
  );
};
